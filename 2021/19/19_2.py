        ...
        for tmpFixedPoint in scanners[scanidx]:
            ...
            for transform in tmpVectors:
                if len(boardVectors & tmpVectors[transform]) > 10:
                    ...
                    newScanPt = scanners[scanidx][tmpFixedPoint].orbit()[transform]
                    scanlocs[scanidx] = newScanPt.vector(fixedPoint)
                    break
            if found:
                break
        if found:
...
distances = set()
for scanIdxA in scanlocs:
    for scanIdxB in scanlocs:
        if scanIdxA != scanIdxB:
            vec = scanlocs[scanIdxA].vector(scanlocs[scanIdxB])
            distances.add(abs(vec.a) + abs(vec.b) + abs(vec.c))
